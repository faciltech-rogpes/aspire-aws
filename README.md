...
**Assertions assíncronas usam polling, nunca `Task.Delay`.** O `PollingHelper.WaitUntilAsync` repete uma condição com intervalo configurável até timeout. Para testes negativos (verificar que algo _não_ acontece), há `AssertNeverAsync`.
